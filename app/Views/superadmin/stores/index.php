<?php
$rows = $rows ?? [];
?>

<style>
.sa-stores {
    width: 100%;
}

.sa-stores * {
    box-sizing: border-box;
}

.sa-header {
    display: flex;
    justify-content: space-between;
    align-items: flex-start;
    gap: 1rem;
    margin-bottom: 1.25rem;
}

.sa-title .sa-eyebrow {
    display: inline-flex;
    margin-bottom: .4rem;
    font-size: .72rem;
    font-weight: 800;
    letter-spacing: .08em;
    color: #378add;
    text-transform: uppercase;
}

.sa-title h2 {
    margin: 0;
    font-size: 1.35rem;
    font-weight: 700;
    color: var(--color-text-primary, #111827);
}

.sa-title p {
    margin: .35rem 0 0;
    font-size: .875rem;
    color: var(--color-text-secondary, #6b7280);
}

.sa-summary-grid {
    display: grid;
    grid-template-columns: repeat(4, minmax(0, 1fr));
    gap: 1rem;
    margin-bottom: 1rem;
}

.sa-summary-card {
    background: var(--color-background-primary, #fff);
    border: 1px solid var(--color-border-tertiary, rgba(0, 0, 0, .10));
    border-radius: 1rem;
    padding: 1rem;
    min-height: 105px;
    transition: transform .18s ease, border-color .18s ease, box-shadow .18s ease;
}

.sa-summary-card:hover {
    transform: translateY(-2px);
    border-color: rgba(55, 138, 221, .25);
    box-shadow: 0 10px 24px rgba(15, 23, 42, .06);
}

.sa-summary-label {
    display: block;
    font-size: .76rem;
    color: var(--color-text-secondary, #6b7280);
    margin-bottom: .35rem;
}

.sa-summary-value {
    font-size: 1.25rem;
    font-weight: 800;
    color: var(--color-text-primary, #111827);
    line-height: 1.25;
    word-break: break-word;
}

.sa-summary-sub {
    display: block;
    margin-top: .35rem;
    font-size: .72rem;
    color: var(--color-text-tertiary, #9ca3af);
}

.sa-panel {
    background: var(--color-background-primary, #fff);
    border: 1px solid var(--color-border-tertiary, rgba(0, 0, 0, .10));
    border-radius: 1rem;
    overflow: hidden;
}

.sa-panel-header {
    padding: 1rem 1.1rem;
    border-bottom: 1px solid var(--color-border-tertiary, rgba(0, 0, 0, .08));
    display: flex;
    justify-content: space-between;
    align-items: center;
    gap: .75rem;
}

.sa-panel-title {
    font-size: .95rem;
    font-weight: 700;
    color: var(--color-text-primary, #111827);
}

.sa-panel-subtitle {
    margin-top: .2rem;
    font-size: .78rem;
    color: var(--color-text-secondary, #6b7280);
}

.sa-table-wrap {
    width: 100%;
    overflow-x: auto;
}

.sa-table {
    width: 100%;
    border-collapse: collapse;
    font-size: .825rem;
    min-width: 900px;
}

.sa-table th {
    padding: .75rem 1rem;
    text-align: left;
    font-size: .72rem;
    font-weight: 800;
    color: var(--color-text-secondary, #6b7280);
    background: var(--color-background-secondary, #f9fafb);
    border-bottom: 1px solid var(--color-border-tertiary, rgba(0, 0, 0, .08));
    white-space: nowrap;
}

.sa-table td {
    padding: .9rem 1rem;
    border-bottom: 1px solid var(--color-border-tertiary, rgba(0, 0, 0, .06));
    color: var(--color-text-primary, #111827);
    vertical-align: middle;
    white-space: nowrap;
}

.sa-table tbody tr {
    transition: background-color .16s ease;
}

.sa-table tbody tr:hover td {
    background: rgba(55, 138, 221, .045);
}

.sa-table tbody tr:last-child td {
    border-bottom: none;
}

.sa-store-cell {
    display: flex;
    align-items: center;
    gap: .7rem;
    min-width: 0;
}

.sa-store-avatar {
    width: 38px;
    height: 38px;
    border-radius: .85rem;
    background: rgba(55, 138, 221, .11);
    color: #378add;
    display: inline-flex;
    align-items: center;
    justify-content: center;
    flex: 0 0 auto;
    font-size: 1rem;
}

.sa-store-name {
    font-weight: 750;
    color: var(--color-text-primary, #111827);
}

.sa-store-id {
    display: block;
    margin-top: .15rem;
    font-size: .72rem;
    color: var(--color-text-tertiary, #9ca3af);
}

.sa-count {
    font-weight: 700;
}

.sa-money {
    font-weight: 700;
}

.sa-net-pos {
    color: #247244;
    font-weight: 800;
}

.sa-net-neg {
    color: #b42318;
    font-weight: 800;
}

.sa-status-pill {
    display: inline-flex;
    align-items: center;
    gap: .35rem;
    padding: .3rem .65rem;
    border-radius: 999px;
    font-size: .72rem;
    font-weight: 800;
    line-height: 1;
    text-transform: capitalize;
}

.sa-status-dot {
    width: .45rem;
    height: .45rem;
    border-radius: 50%;
}

.sa-status-active {
    background: #eaf7ef;
    color: #247244;
}

.sa-status-active .sa-status-dot {
    background: #22c55e;
}

.sa-status-muted {
    background: #f3f4f6;
    color: #6b7280;
}

.sa-status-muted .sa-status-dot {
    background: #9ca3af;
}

.sa-empty {
    padding: 2rem 1rem;
    text-align: center;
    color: var(--color-text-tertiary, #9ca3af);
}

@media (max-width: 1100px) {
    .sa-summary-grid {
        grid-template-columns: repeat(2, minmax(0, 1fr));
    }
}

@media (max-width: 640px) {
    .sa-header {
        flex-direction: column;
    }

    .sa-summary-grid {
        grid-template-columns: 1fr;
    }

    .sa-panel-header {
        flex-direction: column;
        align-items: flex-start;
    }
}
</style>

<?php
$totalStores = count($rows);
$activeStores = 0;
$totalUsers = 0;
$totalProducts = 0;
$totalTransactions = 0;
$totalRevenue = 0;
$totalExpenses = 0;
$totalNet = 0;

foreach ($rows as $row) {
    if (($row['status'] ?? '') === 'active') {
        $activeStores++;
    }

    $totalUsers += (int) ($row['user_count'] ?? 0);
    $totalProducts += (int) ($row['product_count'] ?? 0);
    $totalTransactions += (int) ($row['transaction_count'] ?? 0);
    $totalRevenue += (float) ($row['revenue'] ?? 0);
    $totalExpenses += (float) ($row['expenses'] ?? 0);
    $totalNet += (float) ($row['net'] ?? 0);
}
?>

<div class="sa-stores">

    <div class="sa-header">
        <div class="sa-title">
            <span class="sa-eyebrow">Platform</span>
            <h2>Manajemen Toko</h2>
            <p>Status dan performa bulan berjalan seluruh cabang.</p>
        </div>
    </div>

    <div class="sa-summary-grid">
        <div class="sa-summary-card">
            <span class="sa-summary-label">Total Toko</span>
            <div class="sa-summary-value"><?= number_format($totalStores) ?></div>
            <span class="sa-summary-sub"><?= number_format($activeStores) ?> toko aktif</span>
        </div>

        <div class="sa-summary-card">
            <span class="sa-summary-label">Total Users</span>
            <div class="sa-summary-value"><?= number_format($totalUsers) ?></div>
            <span class="sa-summary-sub">seluruh cabang</span>
        </div>

        <div class="sa-summary-card">
            <span class="sa-summary-label">Total Produk</span>
            <div class="sa-summary-value"><?= number_format($totalProducts) ?></div>
            <span class="sa-summary-sub">produk terdaftar</span>
        </div>

        <div class="sa-summary-card">
            <span class="sa-summary-label">Net Bulan Ini</span>
            <div class="sa-summary-value <?= $totalNet >= 0 ? 'sa-net-pos' : 'sa-net-neg' ?>">
                <?= formatRupiah($totalNet) ?>
            </div>
            <span class="sa-summary-sub">omzet dikurangi pengeluaran</span>
        </div>
    </div>

    <div class="sa-panel">
        <div class="sa-panel-header">
            <div>
                <div class="sa-panel-title">Daftar Toko</div>
                <div class="sa-panel-subtitle">Ringkasan users, produk, transaksi, omzet, pengeluaran, dan net.</div>
            </div>
        </div>

        <div class="sa-table-wrap">
            <table class="sa-table">
                <thead>
                    <tr>
                        <th>Toko</th>
                        <th>Status</th>
                        <th>Users</th>
                        <th>Produk</th>
                        <th>Transaksi</th>
                        <th>Omzet Bulan Ini</th>
                        <th>Pengeluaran</th>
                        <th>Net</th>
                    </tr>
                </thead>

                <tbody>
                    <?php foreach ($rows as $row): ?>
                        <?php
                        $isActive = ($row['status'] ?? '') === 'active';
                        $net = (float) ($row['net'] ?? 0);
                        ?>
                        <tr>
                            <td>
                                <div class="sa-store-cell">
                                    <div class="sa-store-avatar">
                                        <i class="ti ti-building-store" aria-hidden="true"></i>
                                    </div>

                                    <div>
                                        <div class="sa-store-name">
                                            <?= e($row['store_name'] ?? '-') ?>
                                        </div>
                                        <span class="sa-store-id">
                                            ID #<?= (int) ($row['store_id'] ?? 0) ?>
                                        </span>
                                    </div>
                                </div>
                            </td>

                            <td>
                                <span class="sa-status-pill <?= $isActive ? 'sa-status-active' : 'sa-status-muted' ?>">
                                    <span class="sa-status-dot"></span>
                                    <?= e($row['status'] ?? '-') ?>
                                </span>
                            </td>

                            <td class="sa-count"><?= number_format((int) ($row['user_count'] ?? 0)) ?></td>
                            <td class="sa-count"><?= number_format((int) ($row['product_count'] ?? 0)) ?></td>
                            <td class="sa-count"><?= number_format((int) ($row['transaction_count'] ?? 0)) ?></td>

                            <td class="sa-money">
                                <?= formatRupiah($row['revenue'] ?? 0) ?>
                            </td>

                            <td class="sa-money">
                                <?= formatRupiah($row['expenses'] ?? 0) ?>
                            </td>

                            <td class="<?= $net >= 0 ? 'sa-net-pos' : 'sa-net-neg' ?>">
                                <?= formatRupiah($row['net'] ?? 0) ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>

                    <?php if (empty($rows)): ?>
                        <tr>
                            <td colspan="8" class="sa-empty">
                                Belum ada data toko.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>

</div>